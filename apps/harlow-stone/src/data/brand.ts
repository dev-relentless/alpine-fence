export const BRAND = {
  name: 'Harlow & Stone Landscapes',
  shortName: 'Harlow & Stone',
  tagline: 'Outdoor environments, crafted to endure.',
  description:
    'Harlow & Stone is a premium Utah landscape design-build studio composing complete outdoor environments — full landscape installations, retaining walls, sport courts, hardscape, lighting, and water — designed with intent and built to last.',
  shortDescription:
    'Premium landscape design-build for Utah estates and homes — full installations, retaining walls, sport courts, hardscape, lighting, and water.',
  domain: 'harlowandstonelandscapes.com',
  url: 'https://harlowandstonelandscapes.com',
  email: 'studio@harlowandstonelandscapes.com',
  // TODO: replace with provisioned studio line (SMS-capable). E.164 format for tel:/sms:.
  phone: '(801) 477-0220',
  phoneRaw: '+18014770220',
  smsBody: "Hi Harlow & Stone — I'd like to start a conversation about a project.",
  address: {
    region: 'Utah — Wasatch Front',
    locality: 'Lehi',
    state: 'UT',
    country: 'US',
    serviceArea: [
      'Salt Lake County',
      'Utah County',
      'Summit County',
      'Davis County',
      'Wasatch County',
    ],
    cities: [
      'Salt Lake City', 'Park City', 'Holladay', 'Sandy', 'Draper',
      'Alpine', 'Highland', 'Lehi', 'Cottonwood Heights', 'Bountiful',
      'Farmington', 'Heber City', 'Midway', 'Pleasant Grove', 'Provo',
    ],
  },
  parent: {
    name: 'Alpine Fence & Deck',
    note: 'Part of a Utah construction family with two decades on the Wasatch Front.',
    url: 'https://alpinefenceanddeck.com',
  },
  social: {
    instagram: '',
    pinterest: '',
  },
  founded: '2025',
  priceRange: '$$$$',
} as const;
